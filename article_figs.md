...
```bash
cd trace_generation/scripts
./launch_pred.sh               # 调用 pred_trace_generation.py，生成 *_coord.pkl / *_sphere.pkl / *_pose.pkl
```
- 说明：输出位于 `trace_files/scene_benchmarks/<density>_rs/`，可通过 `--seed`、`--obb-vis`/`--sphere-vis` 控制行为。

3) （可选）从 OBB/link 数据生成球体表示（用于 GNN 流程或需要 Link->Sphere 映射时）
```bash
# 批量生成（按 difficulty 和 id）
./generate_sphere_data.sh --algo gnnmp --start 1 --end 100
```

4) 运行仿真对比并生成 per-difficulty CSV（link vs Sphere）
```bash
