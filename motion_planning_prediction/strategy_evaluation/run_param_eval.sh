...
BASE_NUM_PRED=2
BASE_NUM_BANKS=8
# 可选: dual_port | multi_bank | distri_dual_port | distri_multi_bank
BASE_CHT_TYPE="distri_multi_bank"
COLLISION_TYPE="sphere"
BASE_COPUS_PER_EDGE=4  # 基础场景的 copus_per_edge
FIXED_QNONCOLL_LEN=128  # 固定非碰撞队列长度
# === 遍历列表（其余参数固定为标准场景） ===
SWEEP_NUM_OOCDS=(2 4 8 16 32)
SWEEP_NUM_COPUS=(2 4 8 16 32)
SWEEP_NUM_PRED=(1 2 3 4)
SWEEP_NUM_BANKS=(1 2 4 8 16 32)
...
run_sweep() {
    ...
    for value in "${values_ref[@]}"; do
        ...
        local copus_per_edge="$BASE_COPUS_PER_EDGE"
        ...
        if [ "$sweep_name" == "NUM_OOCDS" ]; then
            ...
            num_copus="$value"
            # NUM_COPUS = 2 时，COPUS_PER_EDGE = 2；否则为 4
            if [ "$value" == "2" ]; then
                copus_per_edge=2
            fi
        ...
        for SCENE in G1 G2 G3 G4 G5; do
            ...
            cht_args=(--cht-type "$BASE_CHT_TYPE")
            if [ "$BASE_CHT_TYPE" = "multi_bank" ] || [ "$BASE_CHT_TYPE" = "distri_multi_bank" ]; then
                cht_args+=(--num-banks "$num_banks")
            fi
...
echo "仿真完成！"
echo "结果已保存至:"
echo "  - $RESULT_DIR/sweep_num_pred_${BASE_CHT_TYPE}_sphere_results.csv"
echo "  - $RESULT_DIR/sweep_num_banks_${BASE_CHT_TYPE}_sphere_results.csv"
# echo "  - $RESULT_DIR/sweep_num_oocds_${BASE_CHT_TYPE}_sphere_results.csv"
# echo "  - $RESULT_DIR/sweep_num_copus_${BASE_CHT_TYPE}_sphere_results.csv"
echo "=========================================="
