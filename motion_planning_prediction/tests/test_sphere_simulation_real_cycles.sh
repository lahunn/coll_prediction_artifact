...
echo "执行命令:"
echo "python3 prediction_simulation_nDOF_sphere_real_cycles.py $THRESHOLD $SAMPLE_RATE $QNONCOLL_MULTIPLIER $DATA_FOLDER $BASE_NAME $NUM_TESTS $ROBOT_NAME"
echo ""

python3 prediction_simulation_nDOF_sphere_real_cycles.py \
    $THRESHOLD \
    $SAMPLE_RATE \
    $QNONCOLL_MULTIPLIER \
    $DATA_FOLDER \
...
if [ $? -eq 0 ]; then
    ...
    echo "========================================"
    echo ""
    echo "结果已保存到: result_files/sphere_results_real_cycles.csv"
    echo ""
    echo "提示:"
    echo "  - 查看CSV结果: cat result_files/sphere_results_real_cycles.csv"
    echo "  - 对比固定周期: diff result_files/sphere_results.csv result_files/sphere_results_real_cycles.csv"
    echo "  - 使用不同参数: $0 -t 0.3 -s 0.2 -q 10 -n 100"
else
    echo ""
    echo "========================================"
    echo "✗ 仿真测试失败"
