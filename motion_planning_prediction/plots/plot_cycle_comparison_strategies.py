...
def plot_cycle_comparison():
    ...
    ax.set_title("Multi-COPU Strategy Comparison: Cycle Count Across Scenes")
    ...
    plt.tight_layout()
    output_path = os.path.join(current_dir, "figs/cycle_comparison_strategies.png")
...
def plot_prediction_impact_comparison():
    ...
    for label, filename in files.items():
        ...
        try:
            ...
            df["Scene_Num"] = df["Scene"].str.extract(r"(\d+)").astype(int)
            df = df.sort_values("Scene_Num")
            if scenes is None: scenes = df["Scene"].tolist()
    ...
    if not cycles_data or scenes is None: return
    ...
    plt.tight_layout()
    output_path = os.path.join(current_dir, "figs/prediction_impact_comparison.png")
    plt.savefig(output_path, dpi=300)
    print(f"Plot saved to {output_path}")
...
def plot_cht_cycles_conflicts_comparison():
    ...
    for label, fname in files.items():
        ...
        conflicts[label] = df["Conflicts"].tolist() if "Conflicts" in df.columns else [0]*len(df)
    ...
    plt.tight_layout()
    output_path = os.path.join(current_dir, "figs/cht_cycles_conflicts_comparison_pred1.png")
    plt.savefig(output_path, dpi=300)
    print(f"Plot saved to {output_path}")
