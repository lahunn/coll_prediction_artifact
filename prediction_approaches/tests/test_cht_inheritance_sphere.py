"""
测试球体CHT继承效果 - 基于coord_hashing_sphere的球体数据
在同一个benchmark内比较:
1. 前半部分训练CHT
2. 后半部分:
   - 策略1: 清零CHT从零开始
...
if len(sys.argv) != 6:
    print(
        "用法: python test_cht_inheritance_sphere.py <密度等级> <坐标量化位数> <半径量化位数> <碰撞阈值> <自由样本采样率>"
    )
    print("示例: python test_cht_inheritance_sphere.py mid 8 6 0.1 0.3")
    sys.exit(1)
...
print("=" * 80)
print("球体CHT继承效果测试 - 同一Benchmark前后半数据对比")
print("=" * 80)
print("\n参数设置:")
print(f"  密度级别: {density_level}")
print(f"  坐标量化位数: {coord_quantize_bits}")
print(f"  半径量化位数: {radius_quantize_bits}")
...
for benchid in range(0, 100):
    ...
    if density_level == "low":
        f = open(
            f"../trace_generation/scene_benchmarks/dens6_rs/obstacles_{benchidstr}_sphere.pkl",
            "rb",
        )
    elif density_level == "mid":
        f = open(
            f"../trace_generation/scene_benchmarks/dens9_rs/obstacles_{benchidstr}_sphere.pkl",
            "rb",
        )
    else:
        f = open(
            f"../trace_generation/scene_benchmarks/dens12_rs/obstacles_{benchidstr}_sphere.pkl",
            "rb",
        )
...
# 控制参数：是否考虑球体半径
consider_radius = False  # 默认仅使用位置信息
...
for benchid in range(0, 100):
    ...
    # 分割数据
    code_first_half = code_pred_quant[:half_samples]
    radius_first_half = radius_pred_quant[:half_samples]
    label_first_half = label_pred[:half_samples]

    code_second_half = code_pred_quant[half_samples:]
    radius_second_half = radius_pred_quant[half_samples:]
    label_second_half = label_pred[half_samples:]
    ...
    # 继承前半部分训练的CHT
    strategy_inherit.inherit_collision_history(strategy_train, rate=0.1)
...
print("\n" + "=" * 80)
print(
    f"最终输出: {density_level}, {coord_quantize_bits}, {radius_quantize_bits}, "
    f"{collision_threshold}, {free_sample_rate}, "
