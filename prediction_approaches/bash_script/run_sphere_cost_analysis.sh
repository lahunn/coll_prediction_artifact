...
COORD_BITS_LIST=(3 4 5 6 7 8)                      # 坐标量化位数
RADIUS_BITS_LIST=(0 1 2 3 4)                    # 半径量化位数
THRESHOLDS=(0.0 0.03125 0.0625 0.125 0.25 0.5 1.0 2.0 4.0)  # 碰撞阈值 (S)
SAMPLE_RATES=(0.01 0.05 0.1 0.3 0.5 0.8 1.0)    # 自由样本采样率 (U)
...
# 写入CSV文件的表头
echo "Density,CoordBits,RadiusBits,Threshold,SampleRate,Precision,Recall,PredCost,BaselineCost,Speedup" > "$OUTPUT_FILE"
    ...
    for radius_bits in "${RADIUS_BITS_LIST[@]}"; do
      for threshold in "${THRESHOLDS[@]}"; do
        for sample_rate in "${SAMPLE_RATES[@]}"; do
          ...
          # 执行Python脚本并捕获输出
          result=$(python coord_hashing_sphere.py "$density" "$coord_bits" "$radius_bits" "$threshold" "$sample_rate" 2>&1)
