...
class SphereHashingPlotter:
    ...
    def plot_variable_analysis(self, variable, fixed_params, 
        ...
        ax1 = axes[0]
        ax1.plot(x_values, pose_precision, 'o-', label='Pose Precision', 
                linewidth=2, markersize=6, color='#2E86AB')
        ax1.plot(x_values, pose_recall, 's-', label='Pose Recall', 
                linewidth=2, markersize=6, color='#A23B72')
        ax1.plot(x_values, elem_precision, 'o--', label='Elem Precision', 
                linewidth=2, markersize=6, color='#2E86AB', alpha=0.6)
        ax1.plot(x_values, elem_recall, 's--', label='Elem Recall', 
    ...
    def plot_multi_curves(self, x_variable, group_variable, fixed_params,
        ...
        for idx, group_val in enumerate(group_values):
            ...
            # Precision & Recall
            axes[0].plot(x_values, pose_precision, 'o-', label=f'{label} (Pose P)', 
                        linewidth=2, markersize=5, color=colors[idx])
            axes[0].plot(x_values, pose_recall, 's-', label=f'{label} (Pose R)', 
                        linewidth=2, markersize=5, color=colors[idx], alpha=0.6)
            axes[0].plot(x_values, elem_precision, 'o--', label=f'{label} (Elem P)', 
                        linewidth=1.5, markersize=4, color=colors[idx], alpha=0.4)
            axes[0].plot(x_values, elem_recall, 's--', label=f'{label} (Elem R)', 
                        linewidth=1.5, markersize=4, color=colors[idx], alpha=0.3)
            
            # Precision vs Recall (Pose level)
            axes[1].plot(pose_recall, pose_precision, 'o-', label=f'{label} (Pose)', 
                        linewidth=2, markersize=5, color=colors[idx])
            axes[1].plot(elem_recall, elem_precision, 's--', label=f'{label} (Elem)', 
                        linewidth=1.5, markersize=4, color=colors[idx], alpha=0.5)
    ...
    def plot_elem_metrics(self, x_variable, group_variable, fixed_params,
                         output_file=None, show_plot=True):
        """
        ...
        for idx, group_val in enumerate(group_values):
            group_df = filtered_df[filtered_df[group_variable] == group_val].sort_values(by=x_variable)
            x_vals = group_df[x_variable].values
            label = f"{group_variable}={group_val}"
            
            axes[0].plot(x_vals, group_df['ElemPrecision'].values, 'o-', 
                        label=label, linewidth=2, markersize=5, color=colors[idx])
            axes[1].plot(x_vals, group_df['ElemRecall'].values, 's-', 
                        label=label, linewidth=2, markersize=5, color=colors[idx])
            axes[2].plot(x_vals, group_df['PredCost'].values, '^-', 
                        label=label, linewidth=2, markersize=5, color=colors[idx])
        ...
        title_parts = [f"{k}={v}" for k, v in fixed_params.items()]
        fig.suptitle(f'Element-Level Metrics - {x_variable} vs {group_variable} ({", ".join(title_parts)})', 
                    fontsize=16, fontweight='bold', y=1.02)
...
def main():
    ...
    print("\n" + "="*60)
    print("Example 5: CoordBits vs Density (fixed RadiusBits=2, Threshold=0.5, SampleRate=1.0)")
    print("="*60)
    plotter.plot_multi_curves(
    ...
    print("\n" + "="*60)
    print("Example 6: RadiusBits vs Density (fixed CoordBits=4, Threshold=0.5, SampleRate=1.0)")
    print("="*60)
    plotter.plot_multi_curves(
    ...
    print("\n" + "="*60)
    print("Example 7: Element Metrics - CoordBits vs Density (fixed RadiusBits=2, Threshold=0.5, SampleRate=1.0)")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Example 8: Element Metrics - RadiusBits vs Density (fixed CoordBits=4, Threshold=0.5, SampleRate=1.0)")
    print("="*60)
    plotter.plot_elem_metrics(
        x_variable='RadiusBits',
        group_variable='Density',
