...
print(low_acc)
ax.bar(group, low_acc, width, color=colors[0], label="Precision (%)")
group = [x + 1 for x in group]
ax.bar(group, low_cov, width, color=colors[1], label="Recall (%)")
group = [x + 1 for x in group]
ax.bar(group, low_cost_scale, width, color=colors[1], label="Compute (%)")
...
plt.text(1, -20, "Baseline", ha="center", va="top", fontsize=36, color=colors[0])
plt.text(
    23,
    -20,
...
group = list(range(0, 45, 5))
width = 1
ax.bar(group, mid_acc, width, color=colors[0], label="Accuracy (%)")
group = [x + 1 for x in group]
ax.bar(group, mid_cov, width, color=colors[1], label="Coverage (%)")
group = [x + 1 for x in group]
ax.bar(group, mid_cost_scale, width, color=colors[1], label="Compute (%)")
# ax.legend(loc="upper center")
ax.set_xticks([i - 1 for i in group])
...
group = list(range(0, 45, 5))
width = 1
ax.bar(group, high_acc, width, color=colors[0], label="Accuracy (%)")
group = [x + 1 for x in group]
ax.bar(group, high_cov, width, color=colors[1], label="Coverage (%)")
group = [x + 1 for x in group]
ax.bar(group, high_cost_scale, width, color=colors[1], label="Compute (%)")
# ax.legend(loc="upper center")
ax.set_xticks([i - 1 for i in group])
