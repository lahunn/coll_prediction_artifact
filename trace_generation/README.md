...
1.随机场景随机pose,测试预测策略的精确率和召回率
  ...
  3.需要修改prediction_approaches/coord_hashing_sphere.py和prediction_approaches/coord_hashing.py中使用的obb和sphere数量
    通过运行prediction_approaches/bash_script/run_sphere_cost_analysis.sh
...
2.实际碰撞检测算法,测试预测策略的精确率和召回率
  ...
  3.需要修改motion_planning_prediction/prediction_simulation_nDOF_sphere.py和
    motion_planning_prediction/prediction_simulation_nDOF.py中使用的obb和sphere数量
    运行motion_planning_prediction/test_sphere_obb_simulation.sh脚本,
    进行基于硬件结构的仿真测试