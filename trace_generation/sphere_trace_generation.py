...
def load_sphere_config():
    ...
    """
    config_path = "../configs/robot/spheres/iiwa_allegro.yml"
    try:
        ...
        return {
            'iiwa7_link_0': [
                {"center": [0.0, 0.0, 0.05], "radius": 0.10}
            ],
            'iiwa7_link_1': [
...
def get_spheres(world, qbase, sphere_config):
    ...
    link_names = ['iiwa7_link_0', 'iiwa7_link_1', 'iiwa7_link_2', 'iiwa7_link_3', 
...
def check_sphere_collision(sphere_center, sphere_radius, world, robot_index, num_obstacles):
    """
    检查单个球体与环境障碍物的碰撞
...
def main():
    # 解析命令行参数
    if len(sys.argv) != 4:
        print("Usage: python sphere_trace_generation.py <numqueries> <foldername> <filenumber>")
        sys.exit(1)
    ...
    while counter < numqueries:
        ...
        for sphere_info in spheres:
            ...
            # 执行球体碰撞检测
            collision = check_sphere_collision(sphere_center, radius, world, robot.index, num_ob)
    ...
    print(f"Sphere collision rate: {np.sum(yarr_sphere == 0) / len(yarr_sphere) * 100:.2f}%")
