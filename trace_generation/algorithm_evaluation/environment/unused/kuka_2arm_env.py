...
class Kuka2Env:
    ...
    def __init__(
        ...
        map_file="maze_files/kukas_14_3000.pkl",
    ):
    ...
    def reset_env(self, collision=True):
        ...
        if collision:
            self.kukaId = p.loadURDF(
                self.kuka_file, [-0.5, 0, 0], [0, 0, 0, 1], useFixedBase=True
            )
            self.kukaId2 = p.loadURDF(
                self.kuka_file, [0.5, 0, 0], [0, 0, 0, 1], useFixedBase=True
            )
        else:
            self.kukaId = p.loadURDF(
        ...
        self.pose_range = [
            (
                p.getJointInfo(self.kukaId, jointId)[8],
                p.getJointInfo(self.kukaId, jointId)[9],
            )
            for jointId in range(p.getNumJoints(self.kukaId))
        ] * 2
    ...
    def _point_in_free_space(self, state):
        ...
        p.performCollisionDetection()
        if (len(p.getContactPoints(self.kukaId)) == 0) and (
            len(p.getContactPoints(self.kukaId2)) == 0
        ):
