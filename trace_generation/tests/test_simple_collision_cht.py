...
class SimpleCollisionCHTTrainer:
    ...
    def _create_simple_obstacles(self, num_obstacles=10):
        ...
        for i in range(num_obstacles):
            ...
            position = np.random.uniform(
                self.position_range[0], self.position_range[1], 3
            )

            # 随机姿态
            orientation = p.getQuaternionFromEuler(np.random.uniform(0, 2 * np.pi, 3))

            # 创建障碍物
            box_shape = p.createCollisionShape(p.GEOM_BOX, halfExtents=half_extents)
            box_visual = p.createVisualShape(
                p.GEOM_BOX, halfExtents=half_extents, rgbaColor=[0.7, 0.7, 0.7, 1.0]
            )
    ...
    def create_sphere(self, position):
        ...
        sphere_shape = p.createCollisionShape(p.GEOM_SPHERE, radius=self.sphere_radius)
        sphere_visual = p.createVisualShape(
            p.GEOM_SPHERE, radius=self.sphere_radius, rgbaColor=[0.2, 0.8, 0.2, 0.7]
        )
    ...
    def create_obb(self, position, orientation):
        ...
        box_shape = p.createCollisionShape(p.GEOM_BOX, halfExtents=self.obb_halfextents)
        box_visual = p.createVisualShape(
            p.GEOM_BOX, halfExtents=self.obb_halfextents, rgbaColor=[0.2, 0.2, 0.8, 0.7]
        )
    ...
    def check_collision(self, body_id):
        ...
        for obstacle_id in self.obstacle_ids:
            contact_points = p.getClosestPoints(
                bodyA=body_id, bodyB=obstacle_id, distance=0.0
            )
    ...
    def random_position(self):
        """生成随机位置"""
        return np.random.uniform(self.position_range[0], self.position_range[1], 3)
    ...
    def quantize_position(self, position, num_bins=32):
        ...
        quantized = np.floor((position - self.position_range[0]) / bin_size).astype(int)
    ...
    def quantize_orientation(self, quaternion, num_bins=16):
        ...
        # 将欧拉角离散化
        quantized = np.floor(np.array(euler) / (2 * np.pi) * num_bins).astype(int)
    ...
    def train_cht(
        self, num_iterations=20000, num_bins_pos=32, num_bins_ori=16, training_ratio=0.8
    ):
        """训练CHT并统计准确率和召回率"""
    ...
    def _compute_metrics(
        ...
        print(
            f"  训练集碰撞率: {(sphere_train_collisions / len(sphere_train_data)) * 100:.2f}%"
        )
        ...
        # 计算混淆矩阵
        true_positives = sum(
            1 for d in sphere_test_data if d["collision"] and d["prediction"]
        )
        false_positives = sum(
            1 for d in sphere_test_data if not d["collision"] and d["prediction"]
        )
        true_negatives = sum(
            1 for d in sphere_test_data if not d["collision"] and not d["prediction"]
        )
        false_negatives = sum(
            1 for d in sphere_test_data if d["collision"] and not d["prediction"]
        )

        # 计算指标
        sphere_accuracy = (
            (true_positives + true_negatives) / len(sphere_test_data)
        ) * 100
        sphere_precision = (
        ...
        print(
            f"  测试集碰撞率: {(sphere_test_collisions / len(sphere_test_data)) * 100:.2f}%"
        )
        ...
        print(
            f"  训练集碰撞率: {(obb_train_collisions / len(obb_train_data)) * 100:.2f}%"
        )
        ...
        # 计算混淆矩阵
        true_positives = sum(
            1 for d in obb_test_data if d["collision"] and d["prediction"]
        )
        false_positives = sum(
            1 for d in obb_test_data if not d["collision"] and d["prediction"]
        )
        true_negatives = sum(
            1 for d in obb_test_data if not d["collision"] and not d["prediction"]
        )
        false_negatives = sum(
            1 for d in obb_test_data if d["collision"] and not d["prediction"]
        )
        ...
        print(
            f"  测试集碰撞率: {(obb_test_collisions / len(obb_test_data)) * 100:.2f}%"
        )
...
def main():
    ...
    for name, n_obs in densities.items():
        ...
        results = trainer.train_cht(
            num_iterations=20000, num_bins_pos=32, num_bins_ori=16, training_ratio=0.8
        )
        ...
        def compute_metrics_from_test(test_data):
            ...
            tn = sum(
                1 for d in test_data if (not d["collision"]) and (not d["prediction"])
            )
            ...
            coll_rate = (
                (sum(1 for d in test_data if d["collision"]) / total * 100)
                if total > 0
                else 0
            )
            return {
    ...
    for rec in summary:
        ...
        o = rec["obb"]
        print(
            f"  球体 -> Precision: {s['precision']:.2f}%, Recall: {s['recall']:.2f}%, Accuracy: {s['accuracy']:.2f}%, CollRate: {s['coll_rate']:.2f}% (N={s['total']})"
        )
        print(
            f"  OBB   -> Precision: {o['precision']:.2f}%, Recall: {o['recall']:.2f}%, Accuracy: {o['accuracy']:.2f}%, CollRate: {o['coll_rate']:.2f}% (N={o['total']})"
        )
