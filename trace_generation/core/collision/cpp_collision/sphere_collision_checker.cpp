...
namespace collision
{
  ...
  std::tuple<bool, std::vector<int>, std::vector<int>> SphereCollisionChecker::check_collisions(
      const std::vector<std::array<double, 4>> &sphere_coords)
  {
    ...
    std::vector<int> sphere_cycles(n, 0);   // 每个球体的周期数
    ...
    return {any_collision, collision_flags, sphere_cycles};
  }

  std::vector<std::tuple<bool, std::vector<int>, std::vector<int>>>
  SphereCollisionChecker::check_collisions_batch(
      const std::vector<std::vector<std::array<double, 4>>> &batch_coords)
  {
    std::vector<std::tuple<bool, std::vector<int>, std::vector<int>>> results;
