...
def path_cost(path):
    return sum(
        [
            edge_cost(path[node_idx], path[node_idx + 1])
...
def obs_data(env, free, collided):
    if not len(free):
        free.append([0.0 for _ in range(env.config_dim)])
    if not len(collided):
        collided.append([0.0 for _ in range(env.config_dim)])
    free = free[:500]
...
def random_path_smoother(path, eps, env, iter=100):
    ...
    if len(path) > 2:
        for _ in range(iter):
            ...
            if (
                env._state_fp(tuple_to_np(new_node))
                and env._edge_fp(tuple_to_np(new_node), tuple_to_np(path[node_idx - 1]))
                and env._edge_fp(tuple_to_np(new_node), tuple_to_np(path[node_idx + 1]))
            ):
                if (
                    np.linalg.norm(path[node_idx + 1] - new_node)
                    + np.linalg.norm(path[node_idx - 1] - new_node)
                ) < (
                    np.linalg.norm(path[node_idx + 1] - prev_node)
                    + np.linalg.norm(path[node_idx - 1] - prev_node)
                ):
        ...
        try:
            ...
            for index, point in enumerate(path):
                if index == 0 or index == (len(path) - 1):
                    ...
                    if not env._edge_fp(
                        tuple_to_np(path[index - 1]), tuple_to_np(path[index + 1])
                    ):
                        crit_idx.append(index)
            ...
            for prev, next in zip(crit_idx[:-1], crit_idx[1:]):
                ...
                dists, prevs = dijkstra(
                    path[prev : (next + 1)], neighbors, edge_cost, path[prev]
                )
                partial_path = []
                current = path[next]
    ...
    for _ in range(iter):
        ...
        while prune_idx != len(shorten_path):
            ...
            for inter_idx in range(random_idx + 1, random_idx_next):
                path[inter_idx] = tuple(
                    (seg_B - seg_A)
                    * (inter_idx - random_idx)
                    / (random_idx_next - random_idx)
                    + seg_A
                )
            prune_idx += 1
...
def proposed_path_smootherv2(old_path, new_path, env):
    K = int(
        np.ceil(
            (
                np.linalg.norm(np.array(old_path) - np.array(new_path), axis=-1)
                / env.RRT_EPS
            ).max()
        )
    )
    ...
    for _ in range(K):
        ...
        for i, ns in enumerate(zip(path[1:-1], new_path[1:-1])):
            ...
            if not (
                env._edge_fp(next_path[i - 1], next_path[i])
                and env._edge_fp(next_path[i + 1], next_path[i])
            ):
                next_path[i] = path[i]
            else:
...
def model_smooth(model, free, collided, old_path, env, iter=5):
    ...
    for iter_i in range(iter):
        ...
        data.edge_index = torch.cat(
            (
                torch.arange(1, len(old_path)).reshape(1, -1),
                torch.arange(0, len(old_path) - 1).reshape(1, -1),
            ),
            dim=0,
        )
        ...
        old_path, info, feas_info = proposed_path_smootherv2_probe(
            old_path, new_path, env
        )
        if iter_i == 0:
...
if __name__ == "__main__":
    ...
    for index in range(2010, 2012):
        ...
        BIT = BITStar(env)
        nodes, edges, collision, path_length, n_samples, _ = BIT.plan(
            float("inf"), refine_time_budget=0, time_budget=5
        )
