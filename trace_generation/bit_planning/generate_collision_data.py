...
def ensure_dirs(pair_dir: str, collision_dir: str) -> None:
...
def save_pair(pair_dir: str, base: str, index: int, obstacles, configs) -> None:
...
def save_collision(
    env: ModularEnv, collision_dir: str, base: str, n_obs: int, index: int, suffix: str
) -> None:
    coll_filename = f"{base}_{n_obs:02d}obs_{index:04d}_{suffix}.pkl"
...
def process_one_problem(
    ...
    n_obs = len(obstacles) if hasattr(obstacles, "__len") else 0
    ...
    save_pair(pair_dir, base, idx1, obstacles, env_link.collision_env.config_list)

    # 保存 link 碰撞数据
    save_collision(env_link, collision_dir, base, n_obs, idx1, "link")
    ...
    # 保存 sphere 碰撞数据
    save_collision(env_sphere, collision_dir, base, n_obs, idx1, "sphere")
...
def main() -> int:
    ...
    ensure_dirs(args.pair_dir, args.collision_dir)
