...
class KukaEnv:
    ...
    def __init__(
        ...
        self.config_dim = p.getNumJoints(
            self.kukaId, physicsClientId=self.physics_client
        )

        if self.config_dim == 7:
            target = p.getDebugVisualizerCamera(physicsClientId=self.physics_client)[11]
        ...
        self.pose_range = [
            (
                p.getJointInfo(
                    self.kukaId, jointId, physicsClientId=self.physics_client
                )[8],
                p.getJointInfo(
                    self.kukaId, jointId, physicsClientId=self.physics_client
                )[9],
            )
            for jointId in range(
                p.getNumJoints(self.kukaId, physicsClientId=self.physics_client)
            )
        ]
    ...
    def get_robot_points(self, config, end_point=True):
        ...
        for i in range(
            ...
            p.resetJointState(
                self.kukaId, i, config[i], physicsClientId=self.physics_client
            )
        if end_point:
            point = p.getLinkState(
                self.kukaId,
        ...
        for effector in range(self.kukaEndEffectorIndex + 1):
            point = p.getLinkState(
                self.kukaId, effector, physicsClientId=self.physics_client
            )[0]
            point = (point[0], point[1], point[2] - 0.4)
            points.append(point)
        return points
    ...
    def set_config(self, c, kukaId=None):
        if kukaId is None:
            kukaId = self.kukaId
        for i in range(p.getNumJoints(kukaId, physicsClientId=self.physics_client)):
            p.resetJointState(kukaId, i, c[i], physicsClientId=self.physics_client)
        p.performCollisionDetection(physicsClientId=self.physics_client)
    ...
    def plot(self, path, make_gif=False):
        ...
        prev_pos = p.getLinkState(
            self.kukaId, self.kukaEndEffectorIndex, physicsClientId=self.physics_client
        )[0]
        final_pos = p.getLinkState(
            target_kukaId,
            self.kukaEndEffectorIndex,
            physicsClientId=self.physics_client,
        )[0]
        ...
        while True:
            ...
            for data in p.getVisualShapeData(
                new_kuka, physicsClientId=self.physics_client
            ):
                color = list(data[-1])
                color[-1] = 0.5
                p.changeVisualShape(
    ...
    def _point_in_free_space(self, state):
        ...
        for i in range(
            ...
            p.resetJointState(
                self.kukaId, i, state[i], physicsClientId=self.physics_client
            )
        # print(len(p.getContactPoints(self.kukaId,self.kukaEndEffectorIndex)))
        p.performCollisionDetection(physicsClientId=self.physics_client)
        if (
            len(p.getContactPoints(self.kukaId, physicsClientId=self.physics_client))
            == 0
        ):
    ...
    def _point_in_free_space_probe(self, state):
        ...
        for linkid in range(0, self.kukaEndEffectorIndex + 1):
            numpon = len(
                p.getContactPoints(
                    self.kukaId, linkIndexA=linkid, physicsClientId=self.physics_client
                )
            )
            if numpon > 0:
                link_feasible = 0
            ...
            link_info.append(
                p.getLinkState(
                    self.kukaId, linkid, physicsClientId=self.physics_client
                )[0]
            )
